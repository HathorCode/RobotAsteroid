...
namespace tc {
	struct SimpleShader
	{
		...
		ShaderInitError Init(const uint32 vsTextSize, const GLchar* vsText, const uint32 fsTextSize, const GLchar* fsText) {
			...
			for (uint32 i = 0; i < 2; i++) {
				...
				if (compileSuccess == GL_FALSE) {
					...
					shaderErrorLog.str = new char[shaderErrorLog.len];
					GLint fakeLength;
			...
			//Uniforms
			this->mdlViewMatrixLoc = glGetUniformLocation(this->glslProgram, "mdlViewMatrix");
			this->textureSamplerLoc = glGetUniformLocation(this->glslProgram, "textureSampler");
			...
			glVertexAttribPointer(AttribNames::vertexTexCoord,
			                      ...
			                      GL_FALSE,
			                      0,
			                      (GLvoid*)(4 * 2 * sizeof(GLfloat)));	//This offset is calculated from renderer.quadbuffer
		...
		void SetShaderUniforms(const glm::mat3* mdlViewMatrix) const {
			...
			glUniformMatrix3fv(this->mdlViewMatrixLoc, 1, GL_FALSE, &(*mdlViewMatrix)[0][0]);
		}		
	};
}
